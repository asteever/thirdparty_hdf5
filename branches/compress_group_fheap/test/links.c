...

/*-------------------------------------------------------------------------
 * Function:    link_fheap_params
 *
 * Purpose:     Tests modifying the fractal heap creation parameters for
 *              group link storage.  Also tests copying these groups.
 *
 * Return:      Success:        0
 *              Failure:        -1
 *
 ...
 *-------------------------------------------------------------------------
 */
/* Function to compare two H5HF_cparam_t structs for equality.  Returns 0 if
 * equal, 1 if unequal. */
static unsigned
fheap_cparam_cmp(const H5HF_cparam_t *cparam1, const H5HF_cparam_t *cparam2)
{
    /* Check top level fields */
    if(cparam1->version != cparam2->version
            || cparam1->width != cparam2->width
            || cparam1->start_block_size != cparam2->start_block_size
            || cparam1->max_direct_size != cparam2->max_direct_size
            || cparam1->max_index != cparam2->max_index
            || cparam1->start_root_rows != cparam2->start_root_rows
            || !cparam1->checksum_dblocks != !cparam2->checksum_dblocks
            || cparam1->max_man_size != cparam2->max_man_size
            || cparam1->id_len != cparam2->id_len)
        return 1;

    return 0;
} /* end fheap_cparam_cmp */

/* Main test routine */
static int
link_fheap_params(hid_t fapl, hbool_t new_format)
{
    hid_t       fid = -1, gid1 = -1, gid2 = -1, gid_tmp = -1;
    hid_t       gcpl1 = -1, gcpl2 = -1, gcpl_tmp = -1;
    H5HF_cparam_t cparam, cparam_out, cparam_def;
    int         nfilters = 0;
    char	filename[NAME_BUF_SIZE];
    htri_t      tri_ret;
     ...
     * etc., which are important for this test. */
    if(new_format)
        TESTING("group link fractal heap params (w/new group format)")
    else
        TESTING("group link fractal heap params")
    ...
    /* Create gcpls, force use of dense storage */
    if((gcpl1 = H5Pcreate(H5P_GROUP_CREATE)) < 0) TEST_ERROR
    if(H5Pset_link_phase_change(gcpl1, 2, 2) < 0) TEST_ERROR
    if((gcpl2 = H5Pcreate(H5P_GROUP_CREATE)) < 0) TEST_ERROR
    if(H5Pset_link_phase_change(gcpl2, 2, 2) < 0) TEST_ERROR

    /* Get default fheap cparams */
    if(H5Pget_fheap_cparams(gcpl1, H5HF_TYPE_GROUP_LINK, &cparam_def) != FALSE)
        TEST_ERROR

    /* Add custom fheap creation params to gcpls */
    cparam.version = H5HF_CPARAM_VERSION_1;
    cparam.width = 8;
    cparam.start_block_size = 256;
    cparam.max_direct_size = 2048;
    cparam.max_index = 19;
    cparam.start_root_rows = 3;
    cparam.checksum_dblocks = FALSE;
    cparam.max_man_size = 31;
    cparam.id_len = 11;
    if(H5Pset_fheap_cparams(gcpl1, H5HF_TYPE_GROUP_LINK, &cparam) < 0)
        TEST_ERROR
    if(H5Pset_fheap_cparams(gcpl2, H5HF_TYPE_GROUP_LINK, &cparam) < 0)
        TEST_ERROR

    /* Add deflate and checksum filters to gcpl2, if available */
    if((tri_ret = H5Zfilter_avail(H5Z_FILTER_DEFLATE)) < 0) TEST_ERROR
    if(tri_ret) {
        if(H5Pset_deflate(gcpl2, 6) < 0) TEST_ERROR
        nfilters++;
    } /* end if */
    if((tri_ret = H5Zfilter_avail(H5Z_FILTER_FLETCHER32)) < 0) TEST_ERROR
    if(tri_ret) {
        if(H5Pset_fletcher32(gcpl2) < 0) TEST_ERROR
        nfilters++;
    } /* end if */

    /* Create a groups using these parameters, add some soft links to them.
     * Link "l1" will be a tiny object, while "link2" and "link3" will be
     * managed objects. */
    if((gid1 = H5Gcreate2(fid, "group1", H5P_DEFAULT, gcpl1, H5P_DEFAULT)) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid1, "l1", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid1, "link2", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid1, "link3", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if((gid2 = H5Gcreate2(fid, "group2", H5P_DEFAULT, gcpl2, H5P_DEFAULT)) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid2, "l1", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid2, "link2", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid2, "link3", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    ...
    /* Reopen file and groups */
    if((fid = H5Fopen(filename, H5F_ACC_RDWR, fapl)) < 0) TEST_ERROR
    if((gid1 = H5Gopen2(fid, "group1", H5P_DEFAULT)) < 0) TEST_ERROR
    if((gid2 = H5Gopen2(fid, "group2", H5P_DEFAULT)) < 0) TEST_ERROR

    /* Retrieve gcpls, verify cparams and number of filters */
    if((gcpl_tmp = H5Gget_create_plist(gid1)) < 0) TEST_ERROR
    if(H5Pget_fheap_cparams(gcpl_tmp, H5HF_TYPE_GROUP_LINK, &cparam_out)
            != TRUE)
        TEST_ERROR
    if(fheap_cparam_cmp(&cparam_out, &cparam)) TEST_ERROR
    if(H5Pget_nfilters(gcpl_tmp) != 0) TEST_ERROR
    if(H5Pclose(gcpl_tmp) < 0) TEST_ERROR
    if((gcpl_tmp = H5Gget_create_plist(gid2)) < 0) TEST_ERROR
    if(H5Pget_fheap_cparams(gcpl_tmp, H5HF_TYPE_GROUP_LINK, &cparam_out)
            != TRUE)
        TEST_ERROR
    if(fheap_cparam_cmp(&cparam_out, &cparam)) TEST_ERROR
    if(H5Pget_nfilters(gcpl_tmp) != nfilters) TEST_ERROR
    if(H5Pclose(gcpl_tmp) < 0) TEST_ERROR

    /* Now try copying the gcpls, and verify cparams and number of filters */
    if((gcpl_tmp = H5Pcopy(gcpl1)) < 0) TEST_ERROR
    if(H5Pget_fheap_cparams(gcpl_tmp, H5HF_TYPE_GROUP_LINK, &cparam_out)
            != TRUE)
        TEST_ERROR
    if(fheap_cparam_cmp(&cparam_out, &cparam)) TEST_ERROR
    if(H5Pget_nfilters(gcpl_tmp) != 0) TEST_ERROR
    if(H5Pclose(gcpl_tmp) < 0) TEST_ERROR
    if((gcpl_tmp = H5Pcopy(gcpl2)) < 0) TEST_ERROR
    if(H5Pget_fheap_cparams(gcpl_tmp, H5HF_TYPE_GROUP_LINK, &cparam_out)
            != TRUE)
        TEST_ERROR
    if(fheap_cparam_cmp(&cparam_out, &cparam)) TEST_ERROR
    ...
    if(H5Lcreate_soft("/", gid1, "link4", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid2, "link4", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR

    /* Copy group 1 */
    if(H5Ocopy(fid, "group1", fid, "group3", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if((gid_tmp = H5Gopen2(fid, "group3", H5P_DEFAULT)) <0) TEST_ERROR

    /* Verify that all links have been copied */
    if(H5Lexists(gid_tmp, "l1", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid_tmp, "link2", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid_tmp, "link3", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid_tmp, "link4", H5P_DEFAULT) != TRUE) TEST_ERROR

    /* Retrieve gcpl, verify cparams and number of filters */
    if((gcpl_tmp = H5Gget_create_plist(gid_tmp)) < 0) TEST_ERROR
    if(H5Pget_fheap_cparams(gcpl_tmp, H5HF_TYPE_GROUP_LINK, &cparam_out)
            != TRUE)
        TEST_ERROR
    if(fheap_cparam_cmp(&cparam_out, &cparam)) TEST_ERROR
    if(H5Pget_nfilters(gcpl_tmp) != 0) TEST_ERROR
    if(H5Pclose(gcpl_tmp) < 0) TEST_ERROR
    if(H5Gclose(gid_tmp) < 0) TEST_ERROR

    /* Copy group 2 */
    if(H5Ocopy(fid, "group2", fid, "group4", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if((gid_tmp = H5Gopen2(fid, "group4", H5P_DEFAULT)) <0) TEST_ERROR
    ...
    /* Delete 3 links to force the groups back into compact mode */
    if(H5Ldelete(gid1, "link2", H5P_DEFAULT) < 0) TEST_ERROR
    if(H5Ldelete(gid1, "link3", H5P_DEFAULT) < 0) TEST_ERROR
    if(H5Ldelete(gid1, "link4", H5P_DEFAULT) < 0) TEST_ERROR
    if(H5Ldelete(gid2, "link2", H5P_DEFAULT) < 0) TEST_ERROR
    if(H5Ldelete(gid2, "link3", H5P_DEFAULT) < 0) TEST_ERROR
    if(H5Ldelete(gid2, "link4", H5P_DEFAULT) < 0) TEST_ERROR
    ...
    /* Reopen file and groups, add 2 links to each, 1 of which will be a "huge"
     * object. */
    if((fid = H5Fopen(filename, H5F_ACC_RDWR, fapl)) < 0) TEST_ERROR
    if((gid1 = H5Gopen2(fid, "group1", H5P_DEFAULT)) < 0) TEST_ERROR
    if((gid2 = H5Gopen2(fid, "group2", H5P_DEFAULT)) < 0) TEST_ERROR
    if(H5Lcreate_soft("/", gid1, "link2", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/_____________________________________________", gid1,
            "link3", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/", gid2, "link2", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    if(H5Lcreate_soft("/_____________________________________________", gid2,
            "link3", H5P_DEFAULT, H5P_DEFAULT) < 0)
        TEST_ERROR
    ...
    /* Reopen file and groups, verify existence of links */
    if((fid = H5Fopen(filename, H5F_ACC_RDWR, fapl)) < 0) TEST_ERROR
    if((gid1 = H5Gopen2(fid, "group1", H5P_DEFAULT)) < 0) TEST_ERROR
    if((gid2 = H5Gopen2(fid, "group2", H5P_DEFAULT)) < 0) TEST_ERROR
    if(H5Lexists(gid1, "l1", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid1, "link2", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid1, "link3", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid2, "l1", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid2, "link2", H5P_DEFAULT) != TRUE) TEST_ERROR
    if(H5Lexists(gid2, "link3", H5P_DEFAULT) != TRUE) TEST_ERROR

    /* Close file, groups, and plists */
    if(H5Gclose(gid1) < 0) TEST_ERROR
    if(H5Gclose(gid2) < 0) TEST_ERROR
    if(H5Fclose(fid) < 0) TEST_ERROR
    if(H5Pclose(gcpl1) < 0) TEST_ERROR
    ...
    /* Test that H5Pset_fheap_cparams called with default cparams resets the
     * "store fheap cparams" field */
    /* Create default property lists */
    if((gcpl1 = H5Pcreate(H5P_GROUP_CREATE)) < 0) TEST_ERROR
    if((gcpl2 = H5Pcopy(gcpl1)) < 0) TEST_ERROR

    /* Set phase change properties (this causes the ginfo property to be marked
     * as "changed" so we can use H5Pequal after the fheap cparams have been
     * changed back to default */
    if(H5Pset_link_phase_change(gcpl1, 2, 2) < 0) TEST_ERROR
    if(H5Pset_link_phase_change(gcpl2, 2, 2) < 0) TEST_ERROR

    /* Verify that the plists are equal */
    if((tri_ret = H5Pequal(gcpl1, gcpl2)) < 0) TEST_ERROR
    if(!tri_ret) TEST_ERROR

    /* Set some non-default fheap cparams on gcpl2, verify they are not equal */
    if(H5Pset_fheap_cparams(gcpl2, H5HF_TYPE_GROUP_LINK, &cparam) < 0)
        TEST_ERROR
    if((tri_ret = H5Pequal(gcpl1, gcpl2)) < 0) TEST_ERROR
    if(tri_ret) TEST_ERROR

    /* Set the default fheap cparams on gcpl2, verify that it is now equal to
     * gcpl1 */
    if(H5Pset_fheap_cparams(gcpl2, H5HF_TYPE_GROUP_LINK, &cparam_def) < 0)
        TEST_ERROR
    if((tri_ret = H5Pequal(gcpl1, gcpl2)) < 0) TEST_ERROR
    if(!tri_ret) TEST_ERROR
...
main(void)
{
    ...
    for(new_format = FALSE; new_format <= TRUE; new_format++) {
        ...
        nerrors += link_fheap_params(my_fapl, new_format) < 0 ? 1 : 0;
