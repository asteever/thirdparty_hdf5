...

/*-------------------------------------------------------------------------
 * Function:    H5HF_cparam_debug
 *
 * Purpose:     Prints debugging info for a fractal heap's creation
 *              parameters.
 *
 * Return:      Non-negative on success/Negative on failure
 *
 ...
 */
herr_t
H5HF_cparam_debug(const H5HF_cparam_t *cparam, FILE * stream, int indent,
    int fwidth)
{
    FUNC_ENTER_NOAPI_NOINIT_NOFUNC(H5HF_cparam_debug)
    ...
    HDfprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
	      "Starting block size:",
	      (unsigned) cparam->start_block_size);
    HDfprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
	      "Max block size:",
	      (unsigned) cparam->max_direct_size);
    HDfprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
	      "Max index :",
	      cparam->max_index);
    HDfprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
	      "Starting root rows :",
	      cparam->start_root_rows);
    HDfprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
	      "Checksum direct blocks? :",
	      cparam->checksum_dblocks ? "Yes" : "No");
    HDfprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
	      "Max managed size :",
	      (unsigned) cparam->max_man_size);
    HDfprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
	      "id length :",
	      (unsigned) cparam->id_len);

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5HF_cparam_debug() */
