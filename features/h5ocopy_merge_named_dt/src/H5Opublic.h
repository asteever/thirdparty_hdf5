...
#define H5O_COPY_MERGE_NAMED_DTYPE_FLAG (0x0040u)   /* Merge named datatypes in dest file */
...
typedef enum H5O_mndt_search_ret_t {
    H5O_MNDT_SEARCH_ERROR = -1,	/* Abort H5Ocopy */
    H5O_MNDT_SEARCH_CONT,	/* Continue the global search of all named datatypes in the destination file */
    H5O_MNDT_SEARCH_STOP	/* Stop the search, but continue copying.  The named datatype will be copied but not merged. */
} H5O_mndt_search_ret_t;

/* Callback to invoke when completing the search for a matching named datatype from the named dtype list */
typedef H5O_mndt_search_ret_t (*H5O_mndt_search_cb_t)(void *op_data);
