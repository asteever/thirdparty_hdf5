...
/* Key object for skiplist of named datatypes */
typedef struct H5O_copy_search_named_dt_key_t {
    H5T_t               *dt;    /* Datatype */
    unsigned long       fileno; /* File number */
} H5O_copy_search_named_dt_key_t;

/* Callback struct for building a list of named datatypes */
typedef struct H5O_copy_search_named_dt_ud_t {
    H5SL_t      *dst_dt_list;   /* Skip list of named datatypes */
...
static herr_t H5O_copy_header(const H5O_loc_t *oloc_src, H5O_loc_t *oloc_dst /*out */,
    hid_t dxpl_id, unsigned cpy_option, H5O_copy_dtype_merge_list_t *dt_list, H5O_mndt_cb_info_t *mndt_cb);
...
static herr_t H5O_copy_free_named_dt_cb(void *item, void *key, void *op_data);
static int H5O_copy_named_dt_cmp(const void *dt1, const void *dt2);
static herr_t H5O_copy_search_named_dt_cb(hid_t group, const char *name,
    const H5L_info_t *linfo, void *udata);
static htri_t H5O_copy_search_named_dt(H5F_t *file_src, H5O_t *oh_src,
    H5O_loc_t *oloc_dst/*in, out*/, hid_t dxpl_id, H5O_copy_t *cpy_info);
static herr_t H5O_copy_insert_named_dt(H5F_t *file_src, H5O_t *oh_src,
...
/* Declare a free list to manage the H5O_copy_search_named_dt_key_t struct */
H5FL_DEFINE(H5O_copy_search_named_dt_key_t);
    ...
    void **udata /*out*/)
{
    ...
    /* If we are merging named datatypes, check for a match in the destination
     * file now */
    if(cpy_info->merge_named_dt && obj_class->type == H5O_TYPE_NAMED_DATATYPE) {
        ...
        if(fileno_src == fileno_dst) {
            ...
            /* Search for a matching named datatype, building the list if
             * necessary */
            if((merge = H5O_copy_search_named_dt(oloc_src->file, oh_src, oloc_dst, dxpl_id, cpy_info)) < 0)
                HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL, "can't search for matching named datatype")
    ...
    /* If we are merging named datatypes and this is a named datatype, insert
     * the copied datatype into the list of named datatypes in the target file.
     */
    if(cpy_info->merge_named_dt && obj_class->type == H5O_TYPE_NAMED_DATATYPE)
        if(H5O_copy_insert_named_dt(oloc_src->file, oh_src, oloc_dst, dxpl_id, cpy_info) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL, "can't insert named datatype into destination list")
    ...
    hid_t dxpl_id, unsigned cpy_option, H5O_copy_dtype_merge_list_t *dt_list, H5O_mndt_cb_info_t *mndt_cb)
{
    ...
    if((cpy_option & H5O_COPY_MERGE_NAMED_DTYPE_FLAG) > 0)
        cpy_info.merge_named_dt = TRUE;

    /* Add dt_list to copy struct */
    cpy_info.dst_dt_suggestion_list = dt_list;

    cpy_info.mndt_cb = mndt_cb->func;
    cpy_info.mndt_ud = mndt_cb->user_data;
    ...
    if(cpy_info.dst_dt_list)
        H5SL_destroy(cpy_info.dst_dt_list, H5O_copy_free_named_dt_cb, NULL);
    ...
    hid_t ocpypl_id, hid_t lcpl_id)
{
    ...
    /* Retrieve the marge named datatype list */
    if(H5P_get(ocpy_plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &dt_list) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get merge named datatype list")

    /* Get callback info */
    if(H5P_get(ocpy_plist, H5O_CPY_MNDT_SEARCH_CB_NAME, &cb_info) < 0)
    ...
    /* Copy the object from the source file to the destination file */
    if(H5O_copy_header(src_loc->oloc, &new_oloc, dxpl_id, cpy_option, dt_list, &cb_info) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL, "unable to copy object")
...

/*-------------------------------------------------------------------------
 * Function:    H5O_copy_free_named_dt_cb
 *
 * Purpose:     Frees the merge named dt skip list key and object.
 *
 ...
 */
static herr_t
H5O_copy_free_named_dt_cb(void *item, void *_key, void UNUSED *op_data)
{
    ...
    key = H5FL_FREE(H5O_copy_search_named_dt_key_t, key);
    addr = H5FL_FREE(haddr_t, addr);

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5O_copy_free_named_dt_cb */


/*-------------------------------------------------------------------------
 * Function:    H5O_copy_named_dt_cmp
 *
 * Purpose:     Skiplist callback used to compare 2 keys for the merge
 *              named dt list.  Mostly a wrapper for H5T_cmp.
 *
 ...
 */
static int
H5O_copy_named_dt_cmp(const void *_key1, const void *_key2)
{
    const H5O_copy_search_named_dt_key_t *key1 = (const H5O_copy_search_named_dt_key_t *)_key1;
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5O_copy_named_dt_cmp */


/*-------------------------------------------------------------------------
 * Function:    H5O_copy_search_named_dt_attr_cb
 *
 * Purpose:     Callback for H5O_attr_iterate_real from
 *              H5O_copy_search_named_dt_check.  Checks if the attribute's
 *              datatype is committed.  If it is, adds it to the merge
 *              named dt skiplist present in udata if it does not match
 *              any already present.
 *
 * Return:      Non-negative on success/Negative on failure
 *
 ...
 */
static herr_t
H5O_copy_search_named_dt_attr_cb(const H5A_t *attr, void *_udata)
{
    H5O_copy_search_named_dt_ud_t *udata = (H5O_copy_search_named_dt_ud_t *)_udata;
    ...
    if(H5T_committed(dt)) {
        /* Allocate key */
        if(NULL == (key = H5FL_MALLOC(H5O_copy_search_named_dt_key_t)))
            HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed")
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5O_copy_search_named_dt_attr_cb */


/*-------------------------------------------------------------------------
 * Function:    H5O_copy_search_named_dt_check
 *
 * Purpose:     Check if the object at obj_oloc is or contains a reference
 *              to a named datatype.  If it does, adds it to the merge
 *              named dt skiplist present in udata if it does not match
 *              any already present.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5O_copy_search_named_dt_check(H5O_loc_t *obj_oloc,
    H5O_copy_search_named_dt_ud_t *udata)
{
    H5O_copy_search_named_dt_key_t *key = NULL; /* Skiplist key */
    haddr_t     *addr = NULL;           /* Destination address */
    hbool_t     obj_inserted = FALSE;   /* Object inserted into skip list */
    ...
    /* Check if the object is a named datatype, a dataset using a named
        * datatype, or contains an attribute using a named datatype */
    ...
    attr_op.u.lib_op = H5O_copy_search_named_dt_attr_cb;
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5O_copy_search_named_dt_check */


/*-------------------------------------------------------------------------
 * Function:    H5O_copy_search_named_dt_cb
 *
 * Purpose:     H5G_visit callback to add named datatypes to the merge
 *              named dt skiplist.  Mostly a wrapper for
 *              H5O_copy_search_named_dt_check.
 *
 * Return:      Non-negative on success/Negative on failure
 *
 * Programmer:  Neil Fortner
 ...
 */
static herr_t
H5O_copy_search_named_dt_cb(hid_t UNUSED group, const char *name,
    const H5L_info_t *linfo, void *_udata)
{
    H5O_copy_search_named_dt_ud_t *udata = (H5O_copy_search_named_dt_ud_t *)_udata; /* Skip list of dtypes in dest file */
    ...
    if(linfo->type == H5L_TYPE_HARD) {
        ...
        if(H5O_copy_search_named_dt_check(&obj_oloc, udata) < 0)
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5O_copy_search_named_dt_cb */


/*-------------------------------------------------------------------------
 * Function:    H5O_copy_search_named_dt
 *
 * Purpose:     Checks if the named datatype present in oh_src matches any
 ...
 */
static htri_t
H5O_copy_search_named_dt(H5F_t *file_src, H5O_t *oh_src,
    H5O_loc_t *oloc_dst/*in, out*/, hid_t dxpl_id, H5O_copy_t *cpy_info)
{
    H5O_copy_search_named_dt_key_t *key = NULL; /* Skiplist key */
    ...
    if(!cpy_info->dst_dt_list) {
        ...
        if(NULL == (cpy_info->dst_dt_list = H5SL_create(H5SL_TYPE_GENERIC, H5O_copy_named_dt_cmp)))
            HGOTO_ERROR(H5E_OHDR, H5E_CANTCREATE, FAIL, "can't create skip list for named datatypes")
    ...
    if(!cpy_info->dst_dt_list_complete) {
        ...
        if(cpy_info->dst_dt_suggestion_list
            ...
            H5O_mndt_search_ret_t search_cb_ret = H5O_MNDT_SEARCH_CONT;

            /* Make callback to see if we should search destination file */
            if(cpy_info->mndt_cb)
                if((search_cb_ret = cpy_info->mndt_cb(cpy_info->mndt_ud)) == H5O_MNDT_SEARCH_ERROR)
                    HGOTO_ERROR(H5E_OHDR, H5E_CALLBACK, FAIL, "callback returned error")

            if(search_cb_ret == H5O_MNDT_SEARCH_CONT) {
                ...
                * keep track of whether it was done before. */
                ...
                /* Traverse the destination file, adding named datatypes to the skip
                * list */
                if(H5G_visit(H5F_FILE_ID(oloc_dst->file), "/", H5_INDEX_NAME, H5_ITER_NATIVE, H5O_copy_search_named_dt_cb, &udata, H5P_LINK_ACCESS_DEFAULT, dxpl_id) < 0)
                    HGOTO_ERROR(H5E_OHDR, H5E_BADITER, FAIL, "object visitation failed")
                cpy_info->dst_dt_list_complete = TRUE;
            } /* end if */
            else
                if(search_cb_ret != H5O_MNDT_SEARCH_STOP)
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5O_copy_search_named_dt */


/*-------------------------------------------------------------------------
 * Function:    H5O_copy_insert_named_dt
 *
 * Purpose:     Insert the named datatype at oloc_dst into the merge named
 ...
 */
static herr_t
H5O_copy_insert_named_dt(H5F_t *file_src, H5O_t *oh_src, H5O_loc_t *oloc_dst,
