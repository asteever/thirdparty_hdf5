...
/* Definitions for merge named dtype list */
#define H5O_CPY_MERGE_NAMED_DT_LIST_SIZE        sizeof(char *)
#define H5O_CPY_MERGE_NAMED_DT_LIST_DEF         NULL
#define H5O_CPY_MERGE_NAMED_DT_LIST_CMP         H5P_ocpy_merge_named_dt_list_cmp
/* Definitions for callback function when completing the search for a matching named datatype from the named dtype list */
#define H5O_CPY_MNDT_SEARCH_CB_SIZE           sizeof(H5O_mndt_cb_info_t)
#define H5O_CPY_MNDT_SEARCH_CB_DEF            {NULL,NULL} 
...
/* General routines */
static H5O_copy_dtype_merge_list_t *H5P_free_merge_named_dtype_list(
...
/* Property callbacks */
static int H5P_ocpy_merge_named_dt_list_cmp(const void *value1, const void *value2, size_t size);
...
H5P_ocpy_reg_prop(H5P_genclass_t *pclass)
{
    ...
    H5O_copy_dtype_merge_list_t *merge_named_dtype_list = H5O_CPY_MERGE_NAMED_DT_LIST_DEF; /* Default merge named dtype list */
    ...
    if(H5P_register_real(pclass, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, H5O_CPY_MERGE_NAMED_DT_LIST_SIZE, &merge_named_dtype_list, NULL, NULL, NULL, NULL, NULL, H5O_CPY_MERGE_NAMED_DT_LIST_CMP, NULL) < 0)
         HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL, "can't insert property into class")

    /* Register property for callback when completing the search for a matching named datatype from the named dtype list */
    if(H5P_register_real(pclass, H5O_CPY_MNDT_SEARCH_CB_NAME, H5O_CPY_MNDT_SEARCH_CB_SIZE, &mndt_cb, NULL, NULL, NULL, NULL, NULL, NULL, NULL) < 0)
         HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL, "can't insert property into class")
done:
...
H5P_ocpy_copy(hid_t dst_plist_id, hid_t src_plist_id, void UNUSED *copy_data)
{
    ...
    if(H5P_get(src_plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &src_dt_list) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get merge named dtype list")

    /* Make copy of merge named dtype list */
    while(src_dt_list) {
        /* Copy src_dt_list */
     ...
     */
    if(H5P_set(dst_plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &dst_dt_list) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "can't set merge named dtype list")

done:
    if(ret_value < 0) {
        dst_dt_list = H5P_free_merge_named_dtype_list(dst_dt_list);
        if(tmp_dt_list) {
...
H5P_ocpy_close(hid_t ocpypl_id, void UNUSED *close_data)
{
    ...
    /* Get the merge named dtype list property from the old property list */
    if(H5P_get(plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &dt_list) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get merge named dtype list")

    /* Free the merge named dtype list */
    dt_list = H5P_free_merge_named_dtype_list(dt_list);
...

/*-------------------------------------------------------------------------
 * Function:       H5P_ocpy_merge_named_dt_list_cmp
 *
 ...
 */
static int
H5P_ocpy_merge_named_dt_list_cmp(const void *_dt_list1, const void *_dt_list2,
    size_t UNUSED size)
{
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5P_ocpy_merge_named_dt_list_cmp() */


/*-------------------------------------------------------------------------
 * Function:    H5P_free_merge_named_dtype_list
 *
...
H5P_free_merge_named_dtype_list(H5O_copy_dtype_merge_list_t *dt_list)
{
...
} /* H5P_free_merge_named_dtype_list */
...

/*-------------------------------------------------------------------------
 * Function:    H5Padd_merge_named_dtype_path
 *
 * Purpose:     Adds path to the list of paths to search first in the
 *              target file when merging named datatypes during H5Ocopy
 *              (i.e. when using the H5O_COPY_MERGE_NAMED_DTYPE_FLAG flag
 ...
 *
 * Usage:       H5Padd_merge_named_dtype_path(plist_id, path)
 *              hid_t plist_id;                 IN: Property list to copy object
 ...
 */
herr_t
H5Padd_merge_named_dtype_path(hid_t plist_id, const char *path)
{
    H5P_genplist_t *plist;      /* Property list pointer */
    H5O_copy_dtype_merge_list_t *old_list; /* Merge named dtype list currently present */
    ...
    /* Get dtype list */
    if(H5P_get(plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &old_list) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get merge named dtype list")
    ...
    if(H5P_set(plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &new_obj) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "can't set merge named dtype list")
    ...
    FUNC_LEAVE_API(ret_value)
} /* end H5Padd_merge_named_dtype_path() */


/*-------------------------------------------------------------------------
 * Function:    H5Pfree_merge_named_dtype_paths
 *
 * Purpose:     Frees and clears the list of paths created by
 *              H5Padd_merge_named_dtype_path.  A new list may then be
 *              created by calling H5Padd_merge_named_dtype_path again.
 *
 * Usage:       H5Pfree_merge_named_dtype_paths(plist_id)
 *              hid_t plist_id;                 IN: Property list to copy object
 *
 * Return:      Non-negative on success/Negative on failure
 *
 ...
 */
herr_t
H5Pfree_merge_named_dtype_paths(hid_t plist_id)
{
    H5P_genplist_t *plist;      /* Property list pointer */
    ...
    if(H5P_set(plist, H5O_CPY_MERGE_NAMED_DT_LIST_NAME, &dt_list) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "can't set merge named dtype list")

done:
    FUNC_LEAVE_API(ret_value)
} /* end H5Pfree_merge_named_dtype_paths() */


/*-------------------------------------------------------------------------
 * Function:    H5Pset_mndt_search_cb
 *
 * Purpose:     Set the callback function when a matching named datatype is not found
 *		from the list of paths stored in the object copy property list.
 * 		H5Ocopy will invoke this callback before searching all named datatypes
 *		at destination.
 *
 * Usage:       H5Pset_mndt_search_cb(plist_id, H5O_mndt_search_cb_t func, void *op_data)
 *              hid_t plist_id;                 IN: Property list to copy object
 *              H5O_mndt_search_cb_t func;      IN: The callback function
 ...
 */
herr_t
H5Pset_mndt_search_cb(hid_t plist_id, H5O_mndt_search_cb_t func, void *op_data)
{
    H5P_genplist_t *plist;      	/* Property list pointer */
    ...
    /* Set callback info */
    if(H5P_set(plist, H5O_CPY_MNDT_SEARCH_CB_NAME, &cb_info) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "can't set callback info")

done:
    FUNC_LEAVE_API(ret_value)
} /* end H5Pset_mndt_search_cb() */


/*-------------------------------------------------------------------------
 * Function:    H5Pget_mndt_search_cb
 *
 * Purpose:     Retrieves the callback function and user data from the specified 
 *		object copy property list.
 *
 * Usage:       H5Pget_mndt_search_cb(plist_id, H5O_mndt_search_cb_t *func, void **op_data)
 *              hid_t plist_id;                 IN: Property list to copy object
 *		H5O_mndt_search_cb_t *func;	OUT: The callback function
 ...
 */
herr_t
H5Pget_mndt_search_cb(hid_t plist_id, H5O_mndt_search_cb_t *func, void **op_data)
{
    H5P_genplist_t *plist;      /* Property list pointer */
    H5O_mndt_cb_info_t cb_info;	/* Callback info struct */
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    /* Get callback info */
    if(H5P_get(plist, H5O_CPY_MNDT_SEARCH_CB_NAME, &cb_info) < 0)
