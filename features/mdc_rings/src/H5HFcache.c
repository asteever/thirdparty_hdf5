	    ...
	    *clean = FALSE;
		...
		if(0 == (root_iblock_status & H5AC_ES__IS_PROTECTED)) {
		     ...
		     */
                    if(NULL == (root_iblock = (H5HF_indirect_t *)H5AC_protect(f, dxpl_id, H5AC_FHEAP_IBLOCK, root_iblock_addr, 
                                                                              NULL, H5C__READ_ONLY_FLAG, H5AC_RING_US)))
		 ...
		 */
                ...
                if(((root_iblock_status & H5AC_ES__IS_PINNED) == 0) &&
                     ...
                     * we can pass NULL udata.
                     */
                    if(NULL == (iblock = (H5HF_indirect_t *)H5AC_protect(f, dxpl_id, H5AC_FHEAP_IBLOCK, root_iblock_addr, NULL, H5C__READ_ONLY_FLAG, H5AC_RING_US)))
                        HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL, "H5AC_protect() faild.")
                    unprotect_root_iblock = TRUE;
	...
	if(H5F_addr_defined(child_iblock_addr)) {
		 ...
		 * problems, or we think of a better way.
			...
			if(0 == (child_iblock_status & H5AC_ES__IS_PROTECTED)) {
			    ...
			    if(NULL == (child_iblock = (H5HF_indirect_t *) H5AC_protect(f, dxpl_id, H5AC_FHEAP_IBLOCK, child_iblock_addr, NULL, H5C__READ_ONLY_FLAG, H5AC_RING_US)))
                                HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL, "H5AC_protect() faild.")
