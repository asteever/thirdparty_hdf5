...
HDmemset(dblock->blk, 0, dblock->size);
    ...
    if(H5AC_insert_entry(hdr->f, dxpl_id, H5AC_FHEAP_DBLOCK, dblock_addr, dblock, H5AC__NO_FLAGS_SET, H5AC_RING_US) < 0)
    ...
    unsigned flags)
{
    ...
    if(NULL == (dblock = (H5HF_direct_t *)H5AC_protect(hdr->f, dxpl_id, H5AC_FHEAP_DBLOCK, dblock_addr, &udata, flags, H5AC_RING_US)))
