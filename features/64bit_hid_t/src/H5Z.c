...
H5Z_can_apply_direct(const H5O_pline_t *pline)
{
    ...
    /* Make "can apply" callbacks for filters in pipeline */
    if(H5Z_prelude_callback(pline, (hid_t)FAIL, (hid_t)FAIL, (hid_t)FAIL, H5Z_PRELUDE_CAN_APPLY) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_CANAPPLY, FAIL, "unable to apply filter")
...
H5Z_set_local_direct(const H5O_pline_t *pline)
{
    ...
    /* Make "set local" callbacks for filters in pipeline */
    if(H5Z_prelude_callback(pline, (hid_t)FAIL, (hid_t)FAIL, (hid_t)FAIL, H5Z_PRELUDE_SET_LOCAL) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_SETLOCAL, FAIL, "local filter parameters not set")
